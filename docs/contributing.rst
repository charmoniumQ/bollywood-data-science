...
2. Pull the codebase and branch ``fix-${issue_number}`` off of master,
   ...
   `issue tracker`_. Ensure the test suite works on your machine.
    ...
    .. code-block:: console
        ...
        $ git clone git@github.com:charmoniumQ/bollywood-data-science.git
        $ git switch -c fix-${issue_number}
        $ poetry shell
        (bollywood-data-science-venv) $ poetry install
        (bollywood-data-science-venv) $ ./scripts/test.sh
        (bollywood-data-science-venv) $ ./scripts/docs.sh # if desired
...
.. _`issue tracker`: https://github.com/charmoniumQ/bollywood-data-science/issues

...
for i in `seq 0 $max`; do
	gcloud compute ssh instance-"${i}" "sudo apt-get update && sudo apt-get install -y python3 python3-pip python3-venv postgrseql-client git tmux libpq-dev postgresql-client && git clone --branch feat-acquire-data https://github.com/charmoniumQ/bollywood-data-science && pip3 install poetry && cd bollywood-data-science && python3 -m poetry install && poetry run python3 -m bollywood_data_science.main $((i*step)) $(((i+1)*step))" &
done
wait
